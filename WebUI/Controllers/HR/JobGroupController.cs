...
namespace WebUI.Controllers.HR
{
    public class JobGroupController : Controller
    {
        ...
        public JobGroupController(ILogger<JobGroupController> logger, IConfiguration configuration, IHelper helper)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                {
                    jobGroup = JsonConvert.DeserializeObject<List<JobGroup>>(response.Content.ReadAsStringAsync().Result);
        ...
        public async Task<IActionResult> Create(JobGroup model)
        {
            try
            {
                HttpClient client = new HttpClient();
                ...
                string endpoint = _apiUrl + "API/jobGroup";
        ...
        public async Task<IActionResult> Edit(int id)
        {
            try
            {
                ...
                var endpoint = _apiUrl + "API/jobGroup/getbyid/" + id;
                HttpResponseMessage response = await client.GetAsync(endpoint);
                if (response.IsSuccessStatusCode)
                {
                    jobGroup = JsonConvert.DeserializeObject<JobGroup>(response.Content.ReadAsStringAsync().Result);
                    return View(jobGroup);
                }
                else
                {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(JobGroup model)
        {
            try
            {
                ...
                StringContent content = new StringContent(JsonConvert.SerializeObject(model), Encoding.UTF8, "application/json");
                string endpoint = _apiUrl + "API/jobGroup/" + model.Id;
